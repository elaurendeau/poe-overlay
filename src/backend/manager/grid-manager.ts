...
export function validateGridSettings(gridSettings: SettingsGridModel) {
    if (!gridSettings.columnCount || gridSettings.columnCount < 1 || gridSettings.columnCount > 200) {
        throw new Error(`Invalid column count at '${gridSettings.columnCount}'. Shouldn't be null, < 1 or > 20`);
    }

    if (!gridSettings.rowCount || gridSettings.rowCount < 1 || gridSettings.rowCount > 200) {
        throw new Error(`Invalid row count at '${gridSettings.rowCount}'. Shouldn't be null, < 1 or > 20`);
    }
}
