...
import {
  ...
  OVERLAY_DISPLAY_POSITION_EDITOR_WINDOW_KEY,
...
export const toggleCaptureOverlayPositionEditor = ipcMain.on(
  "toggle-overlay-capture-position-editor",
  async (event, overlay: OverlayModel) => {
    logger.debug(
      `SettingsOverlayEditComponent.send -> toggle-overlay-capture-position-editor: ${JSON.stringify(
        overlay
      )}`
    );
    let window =
...
export const toggleDisplayOverlayPositionEditor = ipcMain.on(
  "toggle-overlay-display-position-editor",
  async (event, overlay: OverlayModel) => {
    logger.debug(
      `SettingsOverlayEditComponent.send -> toggle-overlay-display-position-editor: ${JSON.stringify(
        overlay
      )}`
    );
    let window =
      electronComponents.windows[OVERLAY_DISPLAY_POSITION_EDITOR_WINDOW_KEY];
    ...
    logger.debug(
      `Current width ${
        overlay.displayRectangle.endX - overlay.displayRectangle.startX
      }`
    );
    logger.debug(
      `Current height ${
        overlay.displayRectangle.endY - overlay.displayRectangle.startY
      }`
    );
    window.setSize(
      overlay.displayRectangle.endX - overlay.displayRectangle.startX,
